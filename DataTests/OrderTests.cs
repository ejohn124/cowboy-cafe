...
namespace CowboyCafe.DataTests
{
    public class OrderTests
    {
        ...
        public void ShouldGetTheEnumerationOfItems()
        {
            ...
            order.Add(item1);
            order.Add(item2);
            Assert.Collection(order.Items, item => Assert.Equal(item0, item), item => Assert.Equal(item1, item), item => Assert.Equal(item2, item));
        }
        ...
        [InlineData(new double[] { 3.12380983930123 })]
